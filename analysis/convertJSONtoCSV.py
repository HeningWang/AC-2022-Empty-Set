...
filenames = glob.glob("..\\data\\kammertenberg*.json")

header = ""
data = []

#open output file and initialize csv writer for data
write_file = open('..\\data\\kammertenberg.csv', 'w', newline='')
writer = csv.writer(write_file)

#open output file and initialize csv writer for participant info
write_subj_info_file = open('..\\data\\kammertenberg_subj_info.csv', 'w', newline='')
...
for file in filenames:
    id = re.split("kammertenberg",(re.split("\.json", file)[0]))[1]
    #print(id);
    ...
    if trials:
       maintrials = trials[4:len(trials)]
